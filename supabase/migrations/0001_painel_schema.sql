-- ============================================================================
-- TENKA Painel — 0001: schema, helpers e triggers de domínio
-- ============================================================================

-- ---------------------------------------------------------------------------
-- PROFILES — espelho de auth.users com dados do painel
-- ---------------------------------------------------------------------------
...
-- SECURITY DEFINER para poder ser usado dentro de políticas RLS sem recursão.
...
create or replace function public.is_assigned(pid uuid, uid uuid default auth.uid())
returns boolean
language sql stable security definer
set search_path = public
as $$
...
-- Todo usuário novo nasce como colaborador. A promoção a admin acontece
-- somente via SQL (primeiro admin) ou pela Edge Function admin-users.
...
begin
  ...
  -- O e-mail espelha auth.users; nenhum cliente o edita diretamente.
...
begin
  ...
  values (new.id, coalesce(auth.uid(), new.created_by), 'projeto_criado',
...
begin
  if new.archived_at is not null and old.archived_at is null then
    insert into public.project_activity (project_id, actor_id, action, metadata)
    values (new.id, auth.uid(), 'projeto_arquivado', '{}'::jsonb);
    return new;
  end if;
  ...
  if array_length(v_changed, 1) is not null then
    insert into public.project_activity (project_id, actor_id, action, metadata)
    values (new.id, auth.uid(), 'projeto_editado',
...
begin
  ...
  insert into public.project_activity (project_id, actor_id, action, metadata)
  values (new.project_id, coalesce(auth.uid(), new.assigned_by),
...
begin
  if new.body is distinct from old.body then
    insert into public.project_activity (project_id, actor_id, action, metadata)
    values (new.project_id, auth.uid(), 'observacao_editada',
            jsonb_build_object('note_id', new.id));
  end if;
  return new;
end;
...
grant execute on function public.create_project(text, text, bigint, date, text, uuid[]) to authenticated;
